...
public class TestCachedElementKeyListWithPool extends TestCachedElementKeyList
{
			...
			throws IOException
	{
		...
		{
			this.listPool = new CachedVarRecordPoolFactory().create(128, 0);
			this.keyList = new LinkedKeyList(this.listPool, 0);
			this.anotherPool = new AnotherVarRecordPoolFactory().create(60, 0);
		}
		else
		{
			this.listPool = new CachedVarRecordPoolFactory().create(128,
	...
	private class AnotherRecordArrayFactory implements RecordArrayFactory
	{
		...
		public RecordArray create(int slotSize) throws IOException
		{
			return new CachedRecordArray(new PlainRecordArray(
					new RandomFileAccesser(new File("testAnotherPool.txt")),
					slotSize), 1024);
		}
